...
%==========================================================================
    ...
    for exp_id = 1:length(input_control_sequence_exps)
        ...
        % plot results
        ax_cells = ut.plotResults(time, discrete_time, continuous_non_linear_model_global_position, open_loop_u, continuous_non_linear_model_y, path_to_track, parameters, 'figure_name', sprintf('open Loop - exp%02d: state',exp_id),'plot_label','non-linear continuous model');
        ut.plotResults(time, discrete_time, continuous_linear_model_global_position, open_loop_u, continuous_linear_model_y, path_to_track, parameters, 'plot_label','continuous linear model','ax_cells',ax_cells,'animate',animate);
        ut.plotResults(time, discrete_time, discrete_linear_model_global_position, open_loop_u, discrete_linear_model_y, path_to_track, parameters, 'plot_label','discrete linear model','ax_cells',ax_cells,'animate',false);
    end